...
class Info_exchange():
    ...
    def receiver(self):
        ...
        if self.recv_request is None:
            self.recv_request = comm.irecv(source=MPI.ANY_SOURCE, tag=0)
            if MPI.Request.Test(self.recv_request):
                data = self.recv_request.wait()
                self.msgs.append(data)
                self.recv_request = comm.irecv(source=MPI.ANY_SOURCE, tag=0)
        
        #if comm.Iprobe(source=MPI.ANY_SOURCE, tag=0):
        #    not MPI.Request.Test(req_info):
    ...
    def sender(self):
        ...
        msg = (msg_weights, self.alpha)
        ...
        #if MPI.Request.Test(self.recv_request) is False:
            ...
            while not MPI.Request.Test(req_info):
                #if self.recv_request is not None:
                #    if MPI.Request.Test(self.recv_request):
                        #data = self.recv_request.wait()
                        #self.msgs.append(data)
                        #self.recv_request = comm.irecv(source=MPI.ANY_SOURCE, tag=0)
