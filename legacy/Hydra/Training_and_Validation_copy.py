...
class Training_and_Validation():
    def __init__(self,
        ...
        if self.include_pos:
            self.pos_size=len(encoder_pos.classes_)
        if self.include_morph:
            self.morph_size=len(encoder_morph.classes_)
    ...
    def train(self, nth_iter = None, start_time=None):
        ...
        for x in generate_corpus_samples(corpus=self.corpus_train, lemma=self.include_lemma, pos=self.include_pos, morph=self.include_morph, dictionary=self.dictionary,context_size=self.context_size, batch_size=self.batch_size):
            ...
            if context_left != None and context_right != None:
                con_L_vectorised = construct_input_repr(context_left, self.encoder_letters, self.max_len_tok, context=True, reverse=True)
                # must be reversed !
                #print(con_L_vectorised.size())
                #quit()
            ...
            #print(tokens_vectorised.sum())
            #print(torch.isnan(tokens_vectorised).sum(),torch.isnan(con_L_vectorised).sum(),torch.isnan(con_R_vectorised).sum())
            #print(torch.isnan(morphs_vectorised).sum(),torch.isnan(poss_vectorised).sum(),torch.isnan(lemmas_vectorised).sum())
            #quit()
            encoder_output = \
            ...
            if self.include_lemma:
                ...
                encoder_output_lemma = encoder_output.unsqueeze(2).expand(-1,-1,self.max_len_lemma)
                
                decoder_output_lemma = self.model.decoder_lemma(encoder_output_lemma)
                decoder_output_lemma = self.model.final_layer_lemma(decoder_output_lemma.permute(0,2,1)).reshape(-1,self.lemma_size)
                decoder_output_lemma = self.model.logsoftmax(decoder_output_lemma)
                loss_lemmatizer = torch.nn.functional.nll_loss(decoder_output_lemma, lemmas_vectorised.reshape(-1), weight=self.lemma_label_weight)
            ...
            if self.include_pos:
                ...
                encoder_output_pos = encoder_output.unsqueeze(2).expand(-1,-1,self.max_tag_len_seq)
                
                
                decoder_output_pos = self.model.decoder_pos(encoder_output_pos)
                decoder_output_pos = self.model.final_layer_pos(decoder_output_pos.permute(0,2,1)).reshape(-1,self.pos_size)
                decoder_output_pos = self.model.logsoftmax(decoder_output_pos)

                loss_pos += torch.nn.functional.nll_loss(decoder_output_pos, poss_vectorised.reshape(-1), weight=self.pos_label_weight)                
            ...
            if self.include_morph:
                ...
                encoder_output_morph = encoder_output.unsqueeze(2).expand(-1,-1,self.max_tag_len_seq)
                
                decoder_output_morph = self.model.decoder_morph(encoder_output_morph)
                decoder_output_morph = self.model.final_layer_morph(decoder_output_morph.permute(0,2,1)).reshape(-1,self.morph_size)
                decoder_output_morph = self.model.logsoftmax(decoder_output_morph)

                loss_morph += torch.nn.functional.nll_loss(decoder_output_morph, morphs_vectorised.reshape(-1), weight=self.morph_label_weight)
            ...
            #logger.info("{}".format(loss_general_lemmatizer))
            #quit()
    ...
    def valid_dev(self, nth_iter = 'Final', development=False):
        ...
        for x in generate_corpus_samples(corpus=corpus, lemma=self.include_lemma, pos=self.include_pos, morph=self.include_morph, dictionary= self.dictionary,context_size=self.context_size, batch_size=self.batch_size):
            ...
            if self.include_lemma:
                ...
                decoder_output_lemma = self.model.decoder_lemma(encoder_output_lemma)
                decoder_output_lemma = self.model.final_layer_lemma(decoder_output_lemma.permute(0,2,1))
                decoder_output_lemma = self.model.logsoftmax(decoder_output_lemma)
                _, topi = decoder_output_lemma.topk(1)
                output_lemmatizer = topi.detach().data.cpu()

                loss_lemmatizer = torch.nn.functional.nll_loss(decoder_output_lemma.reshape(-1,self.lemma_size), lemmas_vectorised.reshape(-1), weight=self.lemma_label_weight)
            ...
            if self.include_pos:
                ...
                decoder_output_pos = self.model.decoder_pos(encoder_output_pos)
                decoder_output_pos = self.model.final_layer_pos(decoder_output_pos.permute(0,2,1))
                decoder_output_pos = self.model.logsoftmax(decoder_output_pos)
                _, topi = decoder_output_pos.topk(1)
                output_tagger_pos = topi.detach().data.cpu()

                loss_pos = torch.nn.functional.nll_loss(decoder_output_pos.reshape(-1,self.pos_size), poss_vectorised.reshape(-1), weight=self.pos_label_weight)                 
            ...
            if self.include_morph:
                ...
                decoder_output_morph = self.model.decoder_morph(encoder_output_morph)
                decoder_output_morph = self.model.final_layer_morph(decoder_output_morph.permute(0,2,1))
                decoder_output_morph = self.model.logsoftmax(decoder_output_morph)
                _, topi = decoder_output_morph.topk(1)
                output_tagger_morph = topi.detach().data.cpu()

                loss_morph = torch.nn.functional.nll_loss(decoder_output_morph.reshape(-1,self.morph_size), morphs_vectorised.reshape(-1), weight=self.morph_label_weight)
            ...
            if self.include_pos:
                ...
                pos_original = poss_vectorised.data.cpu().view(-1,self.max_tag_len_seq).numpy().tolist()
            ...
            if self.include_morph:   
                ...
                morph_original = morphs_vectorised.data.cpu().view(-1,self.max_tag_len_seq).numpy().tolist()
