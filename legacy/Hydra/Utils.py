...
def arguments_and_configuration():
    ...
    parser.add_argument('--nb_instances', type=lambda x:None if x == 'None' else int(x), nargs='?', default=None,
                        help="the path to the dictionary directory")
    ...
    args.nb_instances is parser.get_default('nb_instances') else params['nb_instances']
...
def check_gpu_and_select_workers(name_host=None, gpu=False, rank=0, size=0, comm=None):
    ...
    if gpu:
        ...
        if rank == 0:
            ...
            for host, proc in hosts_proc.items():
        ...
        '''
        use_cuda = False
        active_workers=list(range(size))
...
def alternate_iterators(*iters): #https://stackoverflow.com/questions/2017895/alternating-between-iterators-in-python  
    ...
    for elt in itertools.chain.from_iterable(
        itertools.zip_longest(*iters,fillvalue=Dummy)):
        if elt is not Dummy:
...
def generate_text_samples(corpus=None, lemma=None, pos=None, morph=None, context_size=10, batch_size=3):
    ...
    random_corpus_loop = list(range(0, len(corpus['token']), 1))
    random.seed(time.time())
    random.shuffle(random_corpus_loop)
    #corpus_loop = [(x,y) for x,y in enumerate(random_corpus_loop)]
    
    for i,x in enumerate(random_corpus_loop):
        
        if i % batch_size == 0:
            tokens, lemmas, poss, morphs, context_left_list, context_right_list = [], [], [], [], [], []
            x_list = random_corpus_loop[i:i+batch_size]#list(range(i, i+batch_size, 1))
            random.shuffle(x_list)
            for x in x_list:
                
                try:
                    y = x#random_corpus_loop[i]
...
def construct_input_repr(sequence=None,
    ...
    #t1 = time.time()
    if context is False:
        input_array = np.full((len(sequence), max_len_tok), '<PAD>', dtype='U13')
        for i, e in enumerate(sequence):
            e = list(e)[:max_len_tok]
            input_array[i,:len(e)] = e
        input_array = input_array.astype(dtype='U13')
    
        input_array = encoder_letters.transform(input_array.reshape(-1)).reshape(len(sequence), max_len_tok)
        return torch.from_numpy(input_array).long()
    else:
        input_array = np.full((len(sequence), len(sequence[0]), max_len_tok), '<PAD>', dtype='U13')
        for i, e in enumerate(sequence):
            if reverse:
                e = e[::-1]
            for j, f in enumerate(e):
                f = list(f)[:max_len_tok]
                input_array[i, j,:len(f)] = f
        input_array = input_array.astype(dtype='U13')
        input_array = encoder_letters.transform(input_array.reshape(-1)).reshape(len(sequence), len(sequence[0]), max_len_tok)
        return torch.from_numpy(input_array).long()
    
    #t2 = time.time()
    #print(t2-t1, 'time elapsed')#np.char.decode(input_array))
    #quit()
    ...
    for tokens in sequence:
        ...
        if not context:
            ...
            #token_ = tokens.split('+')
            #print(token_.encode('utf-8'), "token")
            #for tok in token_:
            token_vectorised = vectorize_token2(token=tokens,
            ...
            list_of_matrices.append(token_vectorised)
...
def construct_output_repr1(sequence=None, encoder=None, max_len_lemma=15):
    ...
    input_array = np.full((len(sequence), max_len_lemma), '<PAD>', dtype='U13')
    for i, e in enumerate(sequence):
        #logger.info("{}".format(e))
        e = list(e)[:max_len_lemma]
        input_array[i,:len(e)] = e
    input_array = input_array.astype(dtype='U13')

    input_array = encoder.transform(input_array.reshape(-1)).reshape(len(sequence), max_len_lemma)
    return torch.from_numpy(input_array).long()
    ...
    EOS = encoder.transform(['<EOS>']).tolist()
    ...
    for element in sequence:
        ...
        for num in range(min(len(element),max_len_lemma-1)):#range(3):
            try:
                chars = []
        ...
        #print(element_repr)
        if len(element_repr) < max_len_lemma:
            #element_repr.extend(EOS)
    ...
    torchy_tensor = torch.from_numpy(list_of_elements).long().to('cpu')
...
def construct_output_repr2(sequence=None, encoder=None, max_tag_len_seq=3):
    ...
    input_array = np.full((len(sequence), max_tag_len_seq), '<PAD>', dtype='U13')
    for i, e in enumerate(sequence):
        #logger.info("{}".format(e))
        #try:
        e = e.split()[:max_tag_len_seq]
        input_array[i,:len(e)] = e
        #except Exception as exc:
        #    raise ValueError("error: {} --- {}".format(exc, e))

    input_array = input_array.astype(dtype='U13')

    input_array = encoder.transform(input_array.reshape(-1)).reshape(len(sequence), max_tag_len_seq)
    return torch.from_numpy(input_array).long()
    ...
    for element in sequence:
        ...
        for elem in element:
            try:
                ...
                #element_repr.extend(empty*max_len_lemma)
        #print(element_repr)
        if len(enhanced_element) < max_tag_len_seq:
            #enhanced_element.extend(EOS)
    ...
    torchy_tensor = torch.LongTensor(list_of_elements).to('cpu')#.view(-1)
...
def inverse_transform(result_list, encoder_lemma):
    for batch_frag in result_list: #single out a batch fragment
        ...
        #print(batch_frag)
        #print(batch_frag.shape)
        #quit()
        lemma_reconstructed = encoder_lemma.inverse_transform(batch_frag.flatten())
...
def inverse_transform_pos_morph(result_list, original_list,encoder):
    ...
    for nb, batch_frag in enumerate(result_list): #single out a batch fragment
        #for position in batch_frag: #single out a position in batch fragment
        result = encoder.inverse_transform(batch_frag.flatten()).tolist()
        #logger.info(original_list[nb])
...
def count_lemmas_validation(result_list, lemma_list, 
    ...
    for nb, element in enumerate(result_list):
        ...
        r += 1.0 if element == lemma_list[nb] else 0.0 #fuzz.ratio(element,lemma_list[nb])/100.0
    ...
    if rank != 0:
        ...
        for x in range(len(active_workers) - 1):
            ...
            for x_id, param in enumerate(train_and_val.model.parameters()):
                        ...
                        if use_cuda:
                            weight = weights[x_id].to('cuda:{}'.format(rank))
...
def tagging(
    ...
    try:
        encoder_letters = load_tools(os.path.join(params['cellarium_folder'], 'Encoders', 'encoder_letters'))
        letters_size = len(encoder_letters.classes_)
        letters_pad = encoder_letters.transform(['<PAD>'])[0]
        if params['include_lemma']:
            encoder_lemma = load_tools(os.path.join(params['cellarium_folder'], 'Encoders', 'encoder_lemma'))
            lemma_size = len(encoder_lemma.classes_)
        if params['include_pos']:
            encoder_pos = load_tools(os.path.join(params['cellarium_folder'], 'Encoders', 'encoder_pos'))
            pos_size = len(encoder_pos.classes_)
        if params['include_morph']:
            encoder_morph = load_tools(os.path.join(params['cellarium_folder'], 'Encoders', 'encoder_morph'))
            morph_size = len(encoder_morph.classes_)

        SOS_lemma, EOS_lemma, PAD_lemma = encoder_lemma.transform(["<SOS>","<EOS>","<PAD>"])
        SOS_pos, EOS_pos, PAD_pos = encoder_pos.transform(["<SOS>","<EOS>","<PAD>"])
        SOS_morph, EOS_morph, PAD_morph = encoder_morph.transform(["<SOS>","<EOS>","<PAD>"])
    ...
    if use_cuda:
        model.to('device:{}'.format(rank))
    
    
    try:
        model.load_state_dict(torch.load(os.path.join(params['cellarium_folder'], "Models", "model_saved_proc_{}".format(0))))
    ...
    for text in os.listdir(folder1):
        ...
        for x in generator_for_tagging(text_in_list, params['batch_size'], params['context_size']):
            ...
            L_context_vectorised = construct_input_repr(context_left_list, encoder_letters, params['max_len_tok'], context=True, reverse=True)
            ...
            encoder_output = \
            model.encode(tokens=tokens_vectorised,
                context_left=L_context_vectorised, 
                context_right=R_context_vectorised
                       )
            ...
            if params['include_lemma']:
                """
                loss_lemmatizer = 0
                decoder_hidden = model.init_hidden_lemmas(encoder_output.size()[0])
                decoder_input = torch.tensor([SOS_lemma]*encoder_output.size()[0])
                if use_cuda:
                    if model.rnn_attn_lemmas.rnn_type == 'lstm':
                ...
                output_lemmatizer = torch.Tensor([PAD_lemma]).expand((encoder_output_lemmas.size(1), params['max_len_lemma'])).long().clone()
                output_lemmatizer_indcs = torch.LongTensor(list(range(encoder_output_lemmas.size(1))))
                
                for indx_position in range(params['max_len_lemma']):


                    if EOS_lemma in output_lemmatizer[:,indx_position]:
                        indcs_out = (output_lemmatizer[:,indx_position] != EOS_lemma).nonzero().reshape(-1)
                        ...
                        if model.rnn_attn_lemmas.rnn_type == 'lstm':
                            ...
                            print(decoder_hidden.size(), output_lemmatizer.size(), indcs_out)
                    ...
                    decoder_output, decoder_hidden = model.rnn_attn_lemmas(
                    ...
                    #print(output_lemmatizer.size(), decoder_input.size())
                    ...
                    #print(decoder_output.size(), target_tensor.size())
                """
                encoder_output_lemma = encoder_output.unsqueeze(2).expand(-1,-1,params['max_len_lemma'])
                
                decoder_output_lemma = model.decoder_lemma(encoder_output_lemma)
                decoder_output_lemma = model.final_layer_lemma(decoder_output_lemma.permute(0,2,1))
                decoder_output_lemma = model.logsoftmax(decoder_output_lemma)
                _, topi = decoder_output_lemma.topk(1)
                output_lemmatizer = topi.detach().data.cpu()
            ...
            if params['include_pos']:
                ...
                """
                loss_pos = 0
                decoder_hidden = model.init_hidden_pos(encoder_output.size()[0])
                decoder_input = torch.tensor([SOS_pos]*encoder_output.size()[0])
                if use_cuda:
                    if model.rnn_attn_pos.rnn_type == 'lstm':
                        decoder_hidden = (decoder_hidden[0].to('cuda:{}'.format(rank)), decoder_hidden[1].to('cuda:{}'.format(rank)))
                    else:
                        decoder_hidden = decoder_hidden.to('cuda:{}'.format(rank))
                    decoder_input = decoder_input.to('cuda:{}'.format(rank))
                ...
                output_tagger_pos = torch.Tensor([PAD_pos]).expand((encoder_output_pos.size(1), params['max_tag_len_seq'])).long().clone()
                output_tagger_pos_indcs = torch.LongTensor(list(range(encoder_output_pos.size(1))))

                for indx_position in range(params['max_tag_len_seq']):
                    
                    if EOS_pos in output_tagger_pos[:,indx_position]:
                        indcs_out = (output_tagger_pos[:,indx_position] != EOS_pos).nonzero().reshape(-1)
                        if indcs_out.nelement() == 0:
                            break  
                        #target_tensor = target_tensor[indcs_out]
                        if model.rnn_attn_pos.rnn_type == 'lstm':
                    ...
                    decoder_output, decoder_hidden = model.rnn_attn_pos(
                ...
                encoder_output_pos = encoder_output.unsqueeze(2).expand(-1,-1,params['max_tag_len_seq'])

                decoder_output_pos = model.decoder_pos(encoder_output_pos)
                decoder_output_pos = model.final_layer_pos(decoder_output_pos.permute(0,2,1))
                decoder_output_pos = model.logsoftmax(decoder_output_pos)
                _, topi = decoder_output_pos.topk(1)
                output_tagger_pos = topi.detach().data.cpu()
                
            if params['include_morph']:
                """
                loss_morph = 0
                decoder_hidden = model.init_hidden_morph(encoder_output.size()[0])
                decoder_input = torch.tensor([SOS_morph]*encoder_output.size()[0])
                if use_cuda:
                    if model.rnn_attn_morph.rnn_type == 'lstm':
                        decoder_hidden = (decoder_hidden[0].to('cuda:{}'.format(rank)), decoder_hidden[1].to('cuda:{}'.format(rank)))
                    else:
                        decoder_hidden = decoder_hidden.to('cuda:{}'.format(rank))
                    decoder_input = decoder_input.to('cuda:{}'.format(rank))
                ...
                output_tagger_morph = torch.Tensor([PAD_morph]).expand((encoder_output_morph.size(1), params['max_tag_len_seq'])).long().clone()
                output_tagger_morph_indcs = torch.LongTensor(list(range(encoder_output_morph.size(1))))

                for indx_position in range(params['max_tag_len_seq']):
                    
                    if EOS_morph in output_tagger_morph[:,indx_position]:
                        indcs_out = (output_tagger_morph[:,indx_position] != EOS_morph).nonzero().reshape(-1)
                        if indcs_out.nelement() == 0:
                            break  
                        #target_tensor = target_tensor[indcs_out]
                        if model.rnn_attn_morph.rnn_type == 'lstm':
                    ...
                    decoder_output, decoder_hidden = model.rnn_attn_morph(
                    ...
                    #print(decoder_output.size(), target_tensor.size())
                """
                encoder_output_morph = encoder_output.unsqueeze(2).expand(-1,-1,params['max_tag_len_seq'])
                
                decoder_output_morph = model.decoder_morph(encoder_output_morph)
                decoder_output_morph = model.final_layer_morph(decoder_output_morph.permute(0,2,1))
                decoder_output_morph = model.logsoftmax(decoder_output_morph)
                _, topi = decoder_output_morph.topk(1)
                output_tagger_morph = topi.detach().data.cpu()
            ...
            if params['include_lemma']:
                ...
                output_lemmatizer = [result.replace('<PAD>','').replace('<EOS>','') for result in inverse_transform(output_lemmatizer.long().numpy(), encoder_lemma)]
            ...
            if params['include_pos']:
                ...
                output_tagger_pos = [result.replace('<PAD>','').replace('<EOS>','') for result in inverse_transform(output_tagger_pos.long().numpy(), encoder_pos)]
            ...
            if params['include_morph']:
                ...
                output_tagger_morph = [result.replace('<PAD>','').replace('<EOS>','') for result in inverse_transform(output_tagger_morph.long().numpy(), encoder_morph)]
